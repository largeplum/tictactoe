...
#include "tictactoe.h"
#include "tictactoe-glue.h" /* generated by dbus-binding-tool */
...
gboolean
tictactoe_keypad(Tictactoe *ttt, guint32 keynum, GError ** error)
{
	int i, j;
	...
	{
		g_print("%d: wrong number, should be 1-9.", keynum);
		return FALSE;
	}

	/* toggle the button associated with keynum */

	i = (keynum - 1) / 3;
	j = (keynum - 1) % 3;

	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (ttt->buttons[i][j]), 
				      !gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON (ttt->buttons[i][j]))); 
...
tictactoe_class_init (TictactoeClass *klass)
{
	...
	/* install GTYPE introspection info */
	dbus_g_object_type_install_info(TICTACTOE_TYPE, &dbus_glib_tictactoe_object_info);
}
